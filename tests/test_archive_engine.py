...
def test_single_folder_archive():
    ...
    info = backend.list(Path("tests/data/single_folder.zip"))

    engine = ArchiveEngine()

    decision = engine.analyze(info)

    assert decision == (ExtractDecision.EXTRACT_HERE)
...
def test_loose_files_archive():
    ...
    assert decision == (ExtractDecision.CREATE_FOLDER)
...
def test_single_file_archive_creates_folder():
    ...
    assert engine.analyze(archive) == (ExtractDecision.CREATE_FOLDER)
...
def test_nested_files_under_single_directory_extract_here():
    ...
    assert engine.analyze(archive) == (ExtractDecision.EXTRACT_HERE)
